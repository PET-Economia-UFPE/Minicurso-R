...
print(meu_df[1, 2])      # Elemento da primeira linha, segunda coluna: "Recife"
print(meu_df[1, ])       # Primeira linha, todas as colunas
print(meu_df[, 1])       # Todas as linhas, primeira coluna (retorna um vetor)
print(meu_df[, "nome"])  # Todas as linhas, coluna "Nome"
print(meu_df[c(1, 3), c("nome", "eh_gato")]) # Linhas 1 e 3

# Extrair colunas como vetores
print(meu_df[["cidade_nascimento"]]) # Similar a meu_df$nome, retorna um vetor
...
# Exemplo: taxa de participação na força de trabalho
...
df <- get_sidra(5944, variable = 4096, period = c("last" = 12)) %>%
  select(df, "Valor", "Unidade de Medida", "Trimestre Móvel (Código)")
...
gini_mean <- filter(coef_gini >= mean(coef_gini))

# 5.3. arrange() -----------------------------------------------------------------------------------------------------
gini_sorted <- arrange(desc(coef_gini))
